...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (3, 210, 160)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        ...
        self.PER = True  # PrTrueioritized Replay (See paper appendix Training), select in priority the elements in the replay buffer which are unexpected for the network
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = gym.make("MsPacman-v4", full_action_space=False)
        if seed is not None:
            self.env.seed(seed)
    ...
    def step(self, action):
        ...
        observation = cv2.resize(observation, (160, 210), interpolation=cv2.INTER_AREA)
